...
bool CPH3::face_is_subdivided(Dart d)const
{
	...
	CPH3 m2(*this);
	m2.current_level_++;
	if (m2.dart_level(phi1(m2, d)) == current_level_ && m2.edge_id(phi1(m2, d)) != m2.edge_id(d))
		subd = true;
	return subd;
}
...
bool CPH3::face_is_subdivided_once(Dart d)const
{
	...
	bool subdOnce = true;
	Dart fit = d;
	CPH3 m2(*this),m3(*this);
	m2.current_level_ = current_level_ + 1;
	m3.current_level_ = current_level_ + 2;
	do
	{
		if (dart_level(phi1(m2, fit)) == current_level_ && edge_id(phi1(m2, fit)) != edge_id(fit))
		{
			subd = true;
			if (dart_level(phi1(m3, fit)) == current_level_ && edge_id(phi1(m3, fit)) != edge_id(fit))
				subdOnce = false;
		}
		++degree;
		fit = phi1(*this, fit);
	...
	if (degree == 3 && subd)
	{
		...
		if (dart_level(phi1(m3, cf)) == current_level_ && edge_id(phi1(m3, cf)) != edge_id(cf))
			subdOnce = false;
	}
...
bool CPH3::volume_is_subdivided(Dart d) const
{
	...
	CPH3 m2(*this);
	m2.current_level_++;
	if (faceAreSubdivided && dart_level(phi<112>(m2, d)) == current_level_ &&
		face_id(phi<112>(m2, d)) != face_id(d))
		subd = true;
