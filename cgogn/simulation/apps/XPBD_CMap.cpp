...
int main(int argc, char** argv)
{
	...
	cgogn::ui::XPBD_Module<Mesh> xp_v(app);
	cgogn::ui::FitVolumeSurface<Surface, Mesh> fvs(app);
	...
	if (have_fine_mesh)
	{
		...
		auto fn = [&]() {
			...
			cgogn::foreach_cell(*m, [&](Vertex v) -> bool {
				if ((cgogn::is_incident_to_boundary(*m, v)))
...
#define IMAX 10
			for (int i = 1; i <= IMAX; i++)
			{
				...
				for (Vertex v : vec_vertices)
				{
					...
					cgogn::foreach_incident_face(*m, v, [&](Face f) -> bool {
						if (!(cgogn::is_incident_to_boundary(*m, f)))
							return true;
						if (cgogn::is_boundary(*m, f.dart))
							f.dart = cgogn::phi3(*m, f.dart);
						n += cgogn::geometry::normal(*m, f, position.get());
						return true;
					});
					n.normalize();
					cgogn::value<Vec3>(*m, n_vert.get(), v) = n;
					Vec3 p = cgogn::value<Vec3>(*m, position.get(), v);
					// Vec3 n = cgogn::value<Vec3>(*m, n_vert.get(), v);
					BVH_Hit h =
					...
					if (h.hit)
					{
						...
						if (Dir.dot(nFace) < 0)
						{
							...
							cgogn::value<Vec3>(*m, position.get(), v) += (double(i) / double(IMAX)) * Dir;
						}
						else
						{
							h.hit = false;
		...
		for (int j = 0; j < nb_subdivision; j++)
		{
			...
			/*for (int l = 0; l < 5; l++)
			{
				...
				{
					m->current_level_ = k;
					vm.changed_connectivity(*m, position.get());
					fvs.update_topo();
					for (int i = 0; i < 1; i++)
					{
						fvs.regularize_surface_vertices(20);
