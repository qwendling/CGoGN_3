...
public:
	...
	void constraint_all_in_one(MAP& m, Volume v, double h)
	{
		// Compute center of mass
		double masse_vol = 0;
		Vec3 cm = Vec3(0, 0, 0);
		...
		// Compute C_H  = det(F) - (1+MU/LAMBDA)
		double C_H = F.determinant() - (1 + LAME_MU / LAME_LAMBDA);

		// Compute C_D = sqrt(tr(F^T*F))
		double C_D = sqrt((F.transpose() * F).trace());
		...
		for (Vertex w : inc_vertices)
		{
			...
			double m_i = value<double>(m, masse_, w);
			Vec3 init_r_i = value<Vec3>(m, init_pos_.get(), w) - value<Vec3>(m, init_cm_, v);
			Vec3 GC_H = m_i * tmp * T_inv_Q * init_r_i;
			value<Vec3>(m, Grad_C_i_, w) = GC_H;
			denum_H += 1.0f / m_i * GC_H.squaredNorm() + alpha_h / (h * h);

			Vec3 GC_C = m_i / r * F * T_inv_Q * init_r_i;
			value<Vec3>(m, Grad_C2_i_, w) = GC_C;
			denum_C += 1.0f / m_i * GC_C.squaredNorm() + alpha_d / (h * h);
		}
		// Compute lambda
		double lambda_H = -C_H / denum_H;

		// Compute lambda
		double lambda_D = -C_D / denum_C;
		...
		for (Vertex w : inc_vertices)
		{
			...
			Vec3 delta_x_i = lambda_H * (1 / m_i) * value<Vec3>(m, Grad_C_i_, w);
			delta_x_i += lambda_D * (1 / m_i) * value<Vec3>(m, Grad_C2_i_, w);
			delta_x_i += F * init_r_i;

			value<Vec3>(m, pos_.get(), w) += delta_x_i / 3.0f;
		}
	}
