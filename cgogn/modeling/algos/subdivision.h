...
#ifndef CGOGN_MODELING_ALGOS_SUBDIVISION_H
#define CGOGN_MODELING_ALGOS_SUBDIVISION_H
	...
	-> std::enable_if_t<std::is_convertible_v<MESH&, CMapBase&>>
{
	...
	for (Dart p : p_point)
	{
		...
		Dart phi323211 = phi<3, 2, 3, 2, 1, 1>(m, p), phi3232111 = phi<3, 2, 3, 2, 1, 1, 1>(m, p),
...
auto butterflySubdivisionVolumeRegular(MESH& m, double angle_threshold,
	...
	-> std::enable_if_t<std::is_convertible_v<MESH&, CMapBase&>>
{
	butterflySubdivisionVolumeRegular(m, angle_threshold, attributes, [&](typename MESH::Vertex) {});
}
...
auto butterflySubdivisionVolumeRegular(MESH& m, double angle_threshold,
	...
	-> std::enable_if_t<std::is_convertible_v<MESH&, CMapBase&>>
{
	butterflySubdivisionVolumeRegular(m, angle_threshold, attributes, after_cut, after_cut, after_cut);
}
